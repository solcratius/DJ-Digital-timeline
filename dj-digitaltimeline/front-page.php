 ...
 */
	...
	$thisID = $post->ID;
	$FP_ID = get_field('tl_select', $thisID)[0];
	$myPass = get_field('login_password', $FP_ID);
	$passOk = true;
	$cookie = "DJ-TIMELINE-LOGIN";

	$CPT = get_post_type($FP_ID);
	$qString = $_SERVER['QUERY_STRING'];
	$catID = array();

	if (get_field('password_protected', $FP_ID))
	{
		$passOk = false;
	...
	if ($CPT == "timeline" && $passOk)
	{
		...
		{
			$typeList_OBJ = get_field('type_list', $FP_ID);
		...
		// echo 'TL-ID:' . $FP_ID . ', EB:' . count($event_objects) . ', Array:' . print_r(array_values($catID));
		$totalM = 12;
		$fy_start = get_field('fy_starts_on', $FP_ID);
		$include_prevQ = get_field('include_prevQ', $FP_ID);
		$include_nextQ = get_field('include_nextQ', $FP_ID);

		$fy_start_m = ltrim(substr($fy_start, 4, 2) - 1, "0");
		if (!$fy_start_m) $fy_start_m  = 0;
		...
		if ($include_nextQ)
		{
			$fy_tmp_m = ltrim(substr($fy_start, 4, 2) - 1, "0") + 3;
			if ($fy_tmp_m > 11)
			{
...
<?php
	if ($CPT == "timeline")
	{
		if ($passOk)
		{
			echo '<div class="main-content"><span class="data">' . get_permalink($FP_ID) . '</span>';
			$id = $FP_ID;
